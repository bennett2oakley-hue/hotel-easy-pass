...
const PHOTO_DB = "hotel-easy-pass-photos";
const PHOTO_STORE = "room_photos";
function openPhotoDB(){return new Promise((resolve,reject)=>{const r=indexedDB.open(PHOTO_DB,1);r.onupgradeneeded=()=>r.result.createObjectStore(PHOTO_STORE,{keyPath:"id"});r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error);});}
function compressRoomPhoto(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=1280,scale=Math.min(1,max/Math.max(img.width,img.height));const canvas=document.createElement("canvas");canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext("2d").drawImage(img,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL("image/jpeg",.78));};img.onerror=reject;img.src=reader.result;};reader.onerror=reject;reader.readAsDataURL(file);});}
async function saveRoomPhotos(type,files){const prepared=[];for(const file of files){prepared.push({id:crypto.randomUUID(),type,data:await compressRoomPhoto(file),name:file.name||"room-photo.jpg",created:new Date().toISOString(),hotel:state.hotel.name||"Hotel stay"});}const db=await openPhotoDB();return new Promise((resolve,reject)=>{const tx=db.transaction(PHOTO_STORE,"readwrite"),store=tx.objectStore(PHOTO_STORE);prepared.forEach(item=>store.put(item));tx.oncomplete=()=>resolve(prepared.length);tx.onerror=()=>reject(tx.error);});}
async function getRoomPhotos(){const db=await openPhotoDB();return new Promise((resolve,reject)=>{const r=db.transaction(PHOTO_STORE,"readonly").objectStore(PHOTO_STORE).getAll();r.onsuccess=()=>resolve((r.result||[]).sort((a,b)=>b.created.localeCompare(a.created)));r.onerror=()=>reject(r.error);});}
async function renderRoomPhotoLog(){const box=document.getElementById("roomPhotoLog");if(!box)return;try{const photos=await getRoomPhotos();if(!photos.length){box.innerHTML="<p class=\"empty\">No room photos saved yet. Your photos stay on this device.</p>";return;}box.innerHTML=photos.slice(0,12).map(p=>"<div class=\"room-photo\"><img src=\""+p.data+"\" alt=\""+esc(p.type==="check-in"?"Room check-in photo":"Room check-out photo")+"\"/><div><strong>"+(p.type==="check-in"?"Check-in":"Check-out")+"</strong><small>"+new Date(p.created).toLocaleString()+"</small></div></div>").join("");}catch(_){box.innerHTML="<p class=\"empty\">Photo storage is unavailable in this browser.</p>";}}
async function saveRoomPhotoSet(type){const input=document.getElementById(type==="check-in"?"checkInPhotos":"checkOutPhotos");const note=document.getElementById(type==="check-in"?"checkInPhotoNote":"checkOutPhotoNote");if(!input?.files?.length)return alert("Choose at least one room photo first.");try{const count=await saveRoomPhotos(type,Array.from(input.files));localStorage.setItem("hep_room_"+type+"_note",note?.value.trim()||"");input.value="";if(note)note.value="";trackEvent("room_photo_saved",{type,count});alert(count+" "+type+" photo"+(count===1?"":"s")+" saved on this device.");renderRoomPhotoLog();}catch(_){alert("The photos could not be saved. Please try again.");}}
  ...
  document.getElementById("app").innerHTML = `
    <div class="hep">
      ...
      <nav class="nav" aria-label="Main navigation">
        ...
        <button onclick="scrollToId('owners')">🏨 Owners</button>
        <button onclick="scrollToId('hotel-branding')">🎨 Hotel Branding</button>
        <button onclick="scrollToId('emergency')">🆘 Emergency</button>
      ...
      <section class="card stay-card" id="stay">
        ...
        <div class="room-documentation"><div class="section-head"><div><span class="kicker">ROOM DOCUMENTATION</span><h3>📸 Check-In & Check-Out Photos</h3></div><span class="partner-badge">PRIVATE ON DEVICE</span></div><p>Take photos when you arrive and again before you leave. Document existing damage, cleanliness or anything you want to remember.</p><div class="room-photo-box"><strong>📥 Check-in</strong><input id="checkInPhotos" type="file" accept="image/*" capture="environment" multiple><input id="checkInPhotoNote" placeholder="Optional note: scratch on desk, stained towel, etc."><button class="secondary wide" onclick="saveRoomPhotoSet('check-in')">Save Check-In Photos</button></div><div class="room-photo-box"><strong>📤 Check-out</strong><input id="checkOutPhotos" type="file" accept="image/*" capture="environment" multiple><input id="checkOutPhotoNote" placeholder="Optional note about room condition at checkout"><button class="secondary wide" onclick="saveRoomPhotoSet('check-out')">Save Check-Out Photos</button></div><div id="roomPhotoLog" class="room-photo-log"></div><small class="muted">Photos stay in this browser and are not uploaded by Hotel Easy Pass. Follow the hotel's normal process for reporting damage or disputes.</small></div>
      </section>
      ...
      <section class="card brand-ready-card" id="hotel-branding">
        <div class="section-head"><div><span class="kicker">COMING LATER</span><h2>🎨 Hotel Branding Studio</h2></div><span class="partner-badge">BRAND READY</span></div>
        <p>We are leaving room for each participating hotel to build a branded guest experience inside Hotel Easy Pass.</p>
        <div class="brand-roadmap">
          <div><strong>🏨 Property identity</strong><span>Hotel logo, name, colors and imagery</span></div>
          <div><strong>✨ Branded specials</strong><span>Promotions styled for each property</span></div>
          <div><strong>🗺️ Guest experience</strong><span>Property-specific directions, amenities and local recommendations</span></div>
          <div><strong>🔗 Direct booking</strong><span>Future booking and campaign links</span></div>
        </div>
        <div class="specials-note"><strong>Built with room to grow:</strong> the current app keeps the traveler experience separate from future hotel branding, so individual properties can be added without redesigning the whole platform.</div>
      </section>

      <section class="card emergency-card" id="emergency">
        <div class="section-head"><div><span class="kicker">WHEN PLANS GO SIDEWAYS</span><h2>🆘 Emergency Travel</h2></div><span class="partner-badge">HELP FAST</span></div>
        <p>Life happens. Hotel Easy Pass keeps the important information close so you can focus on getting help instead of figuring out what to do next.</p>
        <div class="emergency-grid">
          <a class="emergency-action" href="tel:911" onclick="trackEvent('emergency_911')"><strong>📞 Call 911</strong><span>Emergency services</span></a>
          <button class="emergency-action" onclick="openEmergencyDirections()"><strong>🏥 Find medical help</strong><span>Search nearby urgent care and hospitals</span></button>
          <button class="emergency-action" onclick="shareEmergencyInfo()"><strong>📤 Share my trip info</strong><span>Share saved hotel details with someone you trust</span></button>
          <button class="emergency-action" onclick="scrollToId('stay');trackEvent('emergency_hotel_info')"><strong>🏨 Show my hotel</strong><span>Quick access to your saved stay</span></button>
        </div>
        <div class="emergency-note"><strong>Important:</strong> For an immediate emergency, call 911. Hotel Easy Pass is a travel organization tool, not a substitute for emergency services or professional medical care.</div>
      </section>

      <section class="card owner-card" id="owners">
        <div class="section-head"><div><span class="kicker">FOR HOTEL OWNERS</span><h2>🏨 Owner Hub</h2></div><span class="partner-badge">OWNER READY</span></div>
        <p>Give properties a simple way to prepare promotions, guest perks and booking links for future publication.</p>
        <div class="owner-grid">
          <input id="ownerHotel" placeholder="Hotel / property name">
          <input id="ownerContact" placeholder="Manager or contact name">
          <input id="ownerOffer" placeholder="Special offer, e.g. 20% off">
          <input id="ownerDates" placeholder="Offer dates">
        </div>
        <textarea id="ownerDetails" placeholder="Describe the promotion, restrictions, room types, perks or booking instructions..."></textarea>
        <input id="ownerBooking" placeholder="Booking or reservation URL">
        <div class="button-row">
          <button class="primary" onclick="saveOwnerDraft()">💾 Save Offer Draft</button>
          <button class="secondary" onclick="clearOwnerDraft()">Clear</button>
        </div>
        <div id="ownerDraftStatus" class="owner-status"></div>
        <div class="owner-benefits">
          <strong>Designed for owners</strong>
          <span>✓ Simple promotion setup</span><span>✓ Reusable offer drafts</span><span>✓ Direct booking link support</span><span>✓ Ready for future featured placement</span>
        </div>
        <div class="specials-note"><strong>Future partner model:</strong> Hotel Easy Pass can offer free basic listings and optional paid featured placement or qualified booking referrals once live partner onboarding is connected.</div>
      </section>
      ...
      <section class="card trip-command" id="command">
        ...
        <button class="wide ghost" onclick="scrollToId('owners');trackEvent('owner_hub_cta')">🏨 I'm a hotel owner</button>
      </section>
...
function saveOwnerDraft() {
  const draft = {
    hotel: document.getElementById("ownerHotel").value.trim(),
    contact: document.getElementById("ownerContact").value.trim(),
    offer: document.getElementById("ownerOffer").value.trim(),
    dates: document.getElementById("ownerDates").value.trim(),
    details: document.getElementById("ownerDetails").value.trim(),
    booking: document.getElementById("ownerBooking").value.trim()
  };
  if (!draft.hotel || !draft.offer) return alert("Add the hotel name and special offer first.");
  localStorage.setItem("hep_owner_draft", JSON.stringify(draft));
  document.getElementById("ownerDraftStatus").textContent = "✓ Offer draft saved on this device. It is not published yet.";
  trackEvent("owner_offer_draft_saved");
}

function clearOwnerDraft() {
  if (!confirm("Clear this owner offer draft?")) return;
  localStorage.removeItem("hep_owner_draft");
  ["ownerHotel","ownerContact","ownerOffer","ownerDates","ownerDetails","ownerBooking"].forEach(id => { const el=document.getElementById(id); if(el) el.value=""; });
  document.getElementById("ownerDraftStatus").textContent = "";
}

function loadOwnerDraft() {
  const draft = JSON.parse(localStorage.getItem("hep_owner_draft") || "null");
  if (!draft) return;
  ["hotel","contact","offer","dates","details","booking"].forEach(key => {
    const id = "owner" + key.charAt(0).toUpperCase() + key.slice(1);
    const el = document.getElementById(id);
    if (el) el.value = draft[key] || "";
  });
  const status = document.getElementById("ownerDraftStatus");
  if (status) status.textContent = "✓ Saved owner draft loaded.";
}


function openEmergencyDirections() {
  const q = encodeURIComponent("urgent care hospital emergency room");
  window.open("https://www.google.com/maps/search/?api=1&query=" + q, "_blank", "noopener");
  trackEvent("emergency_medical_search");
}

async function shareEmergencyInfo() {
  const stay = state.hotel || {};
  const text = [
    "Hotel Easy Pass emergency travel info",
    stay.name ? "Hotel: " + stay.name : "",
    stay.room ? "Room: " + stay.room : "",
    stay.address ? "Address: " + stay.address : "",
    stay.checkIn ? "Check-in: " + stay.checkIn : "",
    "I may need help. Please contact me directly if you can."
  ].filter(Boolean).join("\n");
  try {
    if (navigator.share) await navigator.share({title:"Emergency Travel Info", text});
    else { await navigator.clipboard.writeText(text); alert("Emergency travel information copied. You can paste it into a text message."); }
    trackEvent("emergency_share_info");
  } catch (e) {}
}
...
.nav{position:sticky;top:0;z-index:5;display:grid;grid-template-columns:repeat(8,1fr);gap:5px;padding:7px;background:rgba(255,255,255,.96);box-shadow:0 2px 12px rgba(0,0,0,.08)}
...
.stay-summary{margin-top:12px;padding:13px;border-radius:13px;background:#f0faf8}.room-documentation{margin-top:14px;padding:15px;border:1px solid #dceceb;border-radius:15px;background:#fbfefe}.room-documentation h3{margin:4px 0 7px;font-size:17px}.room-documentation p{font-size:13px}.room-photo-box{margin-top:10px;padding:12px;border-radius:12px;background:#f2f9f8;border:1px solid #dceceb}.room-photo-box strong{display:block;margin-bottom:8px}.room-photo-box input[type=file]{background:white;padding:9px}.room-photo-box input:not([type=file]){margin-top:7px}.room-photo-log{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.room-photo{border:1px solid #dceceb;border-radius:12px;overflow:hidden;background:white}.room-photo img{width:100%;height:130px;object-fit:cover;display:block}.room-photo div{padding:8px}.room-photo small{display:block;color:#667;font-size:10px;margin-top:3px}.stay-summary small{display:block;margin-top:5px;color:#557070}.room{display:inline-block;margin-left:6px;padding:3px 7px;border-radius:7px;background:#d7eee9;font-size:12px}
