...
async function saveRoomPhotos(type,files){const db=await openPhotoDB();const tx=db.transaction(PHOTO_STORE,"readwrite"),store=tx.objectStore(PHOTO_STORE);for(const file of files){store.put({id:crypto.randomUUID(),type,data:await compressRoomPhoto(file),name:file.name||"room-photo.jpg",created:new Date().toISOString(),hotel:state.hotel.name||"Hotel stay"});}return new Promise((resolve,reject)=>{tx.oncomplete=()=>resolve(files.length);tx.onerror=()=>reject(tx.error);});}
